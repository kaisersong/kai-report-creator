...
**三个好处：**
- 人类可以自然地书写和编辑，无需了解 HTML
- AI 对每个层次使用不同的渲染规则——正文做 Markdown 转换，组件块做模板渲染
- IR 文件可检查、可版本管理，在生成 HTML 之前即可审阅
...
AI 智能体读取第一层即可获得 3 秒全局概览，进入第二层获取章节级理解，仅在需要特定数据时才访问第三层。同样的渐进式披露原则——这次是为机器读取报告而设计的。
